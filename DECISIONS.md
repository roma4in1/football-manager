...
- **Tensions reported per the stop-rule (with evidence, not forced):**
  ...
  fixture-plain/real-sharp 6/7 on that one check). CI note: realism:ci
  (fixture) can land red on a roll — merging needs either acceptance of a
  gate re-alignment (0.55 → ~0.52, rationale: draws entered band, ordering
  held by r=0.86/goal-ratio/ST/GK) or further mechanism work — USER decision.
...
- **e2e 4/5**: the carried-frame share check straddles its >0.5 gate
  (0.48–0.52) — real flights + loose seconds legitimately hold the ball
  un-owned more than the teleport era; the ball-at-feet pin check passes
  ≥95%. Handoff to Phase 2 (motion quality): aerial knockdowns directed to
  teammates instead of always-loose.
