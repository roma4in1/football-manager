-- ════════════════════════════════════════════════════════════════════════════
-- 0001_baseline.sql — FROZEN. DO NOT EDIT THIS FILE, EVER.
--
-- A byte-copy of server/schema.sql as of 2026-08-03, the moment the migration
-- runner was introduced. At that moment the live production database and
-- schema.sql were identical (schema.sql created it once via psql, plus the
-- `accounts` DDL hand-run for accounts-arc phase 1 — which schema.sql already
-- contained). This file pins that shared starting point.
--
-- On a database that ALREADY has this schema (production), the runner ADOPTS
-- this migration: it records it as applied and executes nothing. On an empty
-- database it runs it. Either way every later migration builds on this exact
-- state, so the two paths converge — enforced by the schema-parity check
-- (server/scripts/check-schema-parity.ts, run in CI).
--
-- Editing this file would retroactively change what "baseline" means and
-- break the checksum recorded in schema_migrations. Schema changes go in a
-- NEW migrations/NNNN_*.sql file AND in schema.sql. See migrations/README.md.
-- ════════════════════════════════════════════════════════════════════════════
