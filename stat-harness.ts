 ...
 * {metric, sim_value, target_band, status} rows; every engine PR runs this.
 *
 * Sections:
 ...
 *  3. batch        — n synthetic matches vs calibration bands
 *  4. sweeps       — instruction sliders min→max, directional effects
 *  5. strength     — strong-vs-weak sanity (info only)
 *
 * Usage: node stat-harness.ts [--n 600] [--json]
 * Deterministic: squad generation and fixture seeds derive from a fixed master seed.
 */
...
interface Row { metric: string; sim_value: number | string; target_band: string; status: 'pass' | 'fail' | 'info' }
const report: Row[] = [];

function check(metric: string, value: number, band: [number, number] | null, digits = 3): void {
  report.push({
    metric,
...
function checkBool(metric: string, ok: boolean, detail?: string): void {
  report.push({ metric, sim_value: detail ?? String(ok), target_band: 'true', status: ok ? 'pass' : 'fail' });
}

// ── 1. determinism ───────────────────────────────────────────────────────────

{
  const rng = Rng.fromSeed(`${MASTER_SEED}|det`);
  ...
  checkBool('determinism: same seed byte-identical', JSON.stringify(a) === JSON.stringify(b));
  checkBool('determinism: different seed diverges', JSON.stringify(a) !== JSON.stringify(c));

  // 2. half-time resume
  const { h1, h2 } = a;
  let throws = false;
  try {
    engine.simulateHalf({ fixtureId: 'det', homeClubId: 'H', awayClubId: 'A', half: 2 },
      { home: clubH.squad, away: clubA.squad }, { home: clubH.tactics, away: clubA.tactics }, 'seed-1');
  } catch { throws = true; }
  checkBool('resume: half 2 without resumeState throws', throws);
  checkBool('resume: rng stream continues (state changes)', h1.endState.rngState !== h2.endState.rngState);
  const ps = Object.values(h2.endState.playerState);
  checkBool('resume: minutesPlayed 90 after H2', ps.every((p) => p.minutesPlayed === 90));
  ...
  checkBool('resume: fatigue monotonic across halves', fatigueUp);
  checkBool('resume: score accumulates', h2.endState.score[0] >= h1.endState.score[0] && h2.endState.score[1] >= h1.endState.score[1]);
}
...
{
  const rng = Rng.fromSeed(`${MASTER_SEED}|batch`);
  for (let i = 0; i < N_MATCHES; i++) {
    ...
    const { h1, h2 } = runMatch(`m${i}`, homeClub, awayClub, `batch-${i}`);
...

{
  ...
  check('goals_per_match', mean(totalGoals), [2.6, 2.9]);
  check('zero_zero_share', m.filter((x) => x.goalsH === 0 && x.goalsA === 0).length / m.length, [0.06, 0.09]);
  check('draw_share', m.filter((x) => x.goalsH === x.goalsA).length / m.length, [0.23, 0.27]);
  check('home_win_share', m.filter((x) => x.goalsH > x.goalsA).length / m.length, [0.43, 0.46]);
  check('away_win_share', m.filter((x) => x.goalsA > x.goalsH).length / m.length, [0.28, 0.31]);
  check('shots_per_team_match', mean(perTeam((x) => x.shots)), [11, 14], 2);
  check('sot_per_team_match', mean(perTeam((x) => x.sot)), [3.5, 5], 2);
  check('xg_per_shot', m.reduce((s, x) => s + x.xg[0] + x.xg[1], 0) / m.reduce((s, x) => s + x.shots[0] + x.shots[1], 0), [0.09, 0.12]);
  check('possession_stddev_pts', stddev(m.map((x) => x.possH)), [6, 9], 2);
  check('pass_completion_mean', mean(perTeam((x) => x.passAcc)), [80, 84], 2);
  check('ppda_mean', mean(perTeam((x) => x.ppda)), [10, 13], 2);
  check('fouls_per_team_match', mean(perTeam((x) => x.fouls)), [10, 13], 2);
  check('yellows_per_team_match', mean(perTeam((x) => x.yellows)), [1.8, 2.2], 2);
  check('reds_per_team_match', mean(perTeam((x) => x.reds)), [0.08, 0.12]);
  check('setpiece_goal_share_incl_pens', m.reduce((s, x) => s + x.goalsSetPieceOrPen, 0) / allGoals, [0.25, 0.33]);
  check('headed_goal_share', m.reduce((s, x) => s + x.goalsHeaded, 0) / allGoals, [0.15, 0.2]);
  check('second_half_goal_share', m.reduce((s, x) => s + x.goalsSecondHalf, 0) / allGoals, [0.52, 0.56]);
  check('injuries_per_player_match', m.reduce((s, x) => s + x.injuries, 0) / (m.length * 22), [0.025, 0.04]);
  check('aerial_duels_per_match', mean(m.map((x) => x.aerialTotal)), [30, 50], 1);
  check('offsides_per_team_match (info)', mean(perTeam((x) => x.offsides)), null, 2);
}

// ── 4. instruction sweeps (directional) ──────────────────────────────────────
...
function sweep(name: string, mutateLo: Mutate, mutateHi: Mutate): { lo: MatchMetrics[]; hi: MatchMetrics[] } {
  ...
  for (const end of ['lo', 'hi'] as const) {
    const rng = Rng.fromSeed(`${MASTER_SEED}|sweep|${name}`); // same rng both ends → same squads
...

{
  ...
  checkBool('sweep press↑ → ppda↓', ppdaHi < ppdaLo - 2, `${ppdaHi.toFixed(1)} < ${ppdaLo.toFixed(1)} − 2`);
  checkBool('sweep press↑ → fatigue↑', fatHi > fatLo + 0.02, `${fatHi.toFixed(3)} > ${fatLo.toFixed(3)} + .02`);
}
{
  const s = sweep('risk',
  ...
  checkBool('sweep risk↑ → passAcc↓', accHi < accLo - 1, `${accHi.toFixed(1)} < ${accLo.toFixed(1)} − 1`);
  checkBool('sweep risk↑ → xg/shot↑', xgpsHi > xgpsLo + 0.005, `${xgpsHi.toFixed(3)} > ${xgpsLo.toFixed(3)} + .005`);
}
{
  ...
  checkBool('sweep lineHeight↑ → opponent offsides↑', offHi > offLo + 0.5, `${offHi.toFixed(2)} > ${offLo.toFixed(2)} + .5`);
}
{
  const s = sweep('cross',
  ...
  checkBool('sweep crossBias↑ → aerial duels↑', airHi > airLo + 2, `${airHi.toFixed(1)} > ${airLo.toFixed(1)} + 2`);
  check('sweep crossBias↑ headed share lo→hi (info)', hdHi - hdLo, null);
}

// ── 5. strength sanity (info) ────────────────────────────────────────────────

{
  const rng = Rng.fromSeed(`${MASTER_SEED}|strength`);
  let wins = 0, draws = 0;
  const n = 200;
  for (let i = 0; i < n; i++) {
  ...
  check('strength: q15-home beats q9 win share (info)', wins / n, null);
  check('strength: q15 vs q9 draw share (info)', draws / n, null);
}
...
if (!JSON_ONLY) {
  const w = [46, 12, 14, 6];
  console.log(`\nstat-harness — AggregateEngine, n=${N_MATCHES} matches (+sweeps ${N_SWEEP}×2×4, strength 200)\n`);
  console.log(['metric'.padEnd(w[0]), 'sim'.padEnd(w[1]), 'target'.padEnd(w[2]), 'status'].join(' '));
  console.log('-'.repeat(w[0] + w[1] + w[2] + w[3] + 3));
  for (const r of report) {
    const mark = r.status === 'pass' ? 'PASS' : r.status === 'fail' ? 'FAIL' : 'info';
    console.log([String(r.metric).padEnd(w[0]), String(r.sim_value).padEnd(w[1]), r.target_band.padEnd(w[2]), mark].join(' '));
  }
