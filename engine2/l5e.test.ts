...
// the danger-EV re-allocation regressed this 5/8 -> 1/8 by exposing
  // the uncontestable-flight hole: intendedReceiverId excluded a live
  // pass from every race, so priced cuts never executed. THE STEP-IN
  // (sim.ts) pays the debt — one elected defender attacks the ball's
  // line when his intercept contests the receiver's — and lands 6/8.
